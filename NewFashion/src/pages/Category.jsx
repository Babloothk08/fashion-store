import axios from "axios";
import { useEffect, useState } from "react";
import { Link, useParams } from "react-router-dom";
import { useDispatch, useSelector } from "react-redux";
import { addItem } from "../features/cart/cartSlice";

const Category = () => {

 

  const [data, setData] = useState([]);
  // console.log("ddddddd",data);
  
  const { category, name } = useParams();
  console.log(category, name);

   const dispatch = useDispatch()

   const cart = useSelector((state) => state.cart.item)
   console.log("cart",cart);
   
  // const uniqueCount = item.length

  useEffect(() => {
    const fetchData = async () => {
      const response = await axios.get("http://localhost:5000/data");
      
      const allData = response.data;
      console.log("response",allData);

      const filterData = allData.filter((item) => item.name === name);
      console.log("filter data",filterData);
      

      setData(filterData);
    };
    fetchData();
  }, [category, name]);
 
  return (
    <div className="flex flex-wrap justify-center gap-20 w-full py-18 pt-20">
   
      {data.map((item) => (
        <div
          key={item.id}
          className="rounded-3xl w-75 flex-col overflow-hidden bg-white shadow-lg"
        >
          <Link to={`/detailpage/${item.id}`}>
            <img
              src={item.image}
              alt={item.title}
              className="cursor-pointer transition-transform duration-300 hover:scale-105 rounded-2xl w-full h-99 object-cover"
            />
          </Link>
          <div className="p-4">
            <div className="flex justify-between py-2">
              <h1 className="text-xl font-semibold">{item.title}</h1>
              <div className="flex text-yellow-400 pt-2">⭐⭐⭐⭐☆</div>
            </div>
            <div className="flex justify-between">
              <h1 className="font-semibold text-xl">Price</h1>
              <div className="flex gap-2">
                <p className="line-through">{item.discount}</p>
                <p>{item.price}</p>
              </div>
            </div>
            <div className="flex justify-between mt-3">
              <button className="px-3 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition cursor-pointer" onClick={() => dispatch(addItem(item))}>
                Add To Cart
              </button>
              <h1 className="cursor-pointer px-3 py-2 text-xl">♡</h1>
            </div>
          </div>
        </div>
      ))}
    </div>
  );
};
export default Category;
